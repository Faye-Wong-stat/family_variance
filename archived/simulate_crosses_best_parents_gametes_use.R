...
# best_parent_use <- read.table("select_best_parents_gametes/best_parent_use.txt")
...
effective_loci_number <- as.numeric(args[3])
replic <- as.numeric(args[4])
parents_names_all <- as.character(args[5:14])

# si <- as.numeric(best_parent_use[1, 1])
# h2 <- as.numeric(best_parent_use[1, 2])
# effective_loci_number <- as.numeric(best_parent_use[1, 3])
# replic <- as.numeric(best_parent_use[1, 4])
# parents_names_all <- as.character(best_parent_use[1, 5:14])
...
for (g in 1:nrow(parents_names)){
  ...
  saveRDS(offspring_list,
          paste("simulate_crosses_best_parents_gametes_use/offspring_genotypes/offspring_list_",
                paste(round(si, 2), h2, effective_loci_number, replic, 
  ...
  saveRDS(offsprings_obj, paste("simulate_crosses_best_parents_gametes_use/offspring_family_info_true/", 
                                paste(round(si, 2), h2, effective_loci_number, replic, 
                                      parents_names[g, 1], parents_names[g, 2], 
                                      sep="_"), 
                                "_.rds", 
  ...
  saveRDS(offsprings_obj, paste("simulate_crosses_best_parents_gametes_use/offspring_family_info_pred/", 
                                paste(round(si, 2), h2, effective_loci_number, replic, 
                                      parents_names[g, 1], parents_names[g, 2], 
                                      sep="_"), 
                                "_.rds", 
