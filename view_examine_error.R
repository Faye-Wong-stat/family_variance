...
score_summ$cM <- factor(score_summ$cM, levels=c("(0.01, 0.1]", "(0.1, 1]", "(1, 10]", 
                                                "(10, 100]", ">100"))
...
errors <- data.frame(error_level=rep("real_trios", 5), 
                     cM=c(">100", "(10, 100]", "(1, 10]", 
                          "(0.1, 1]", "(0.01, 0.1]"), 
                     num_compr=comp_real, 
                     num_corr=error_real, 
                     error_rate=(1 - error_real/comp_real))
errors$cM <- factor(c(">100", "(10, 100]", "(1, 10]", 
                      "(0.1, 1]", "(0.01, 0.1]"), 
                    levels=c("(0.01, 0.1]", "(0.1, 1]", "(1, 10]", 
                             "(10, 100]", ">100"))

score_summ <- rbind(score_summ, errors)
score_summ$error_level[score_summ$error_level=="real_trios"] <- "real trios"
...
cols[levels(score_summ$error_level)=="real trios"] <- "black"

p1 <- ggplot(data=score_summ, aes(x=cM, y=error_rate, group=error_level, color=error_level)) + 
  geom_line(linewidth=0.5) + 
  xlab("pairwise distance (cM)") + ylab("fraction of inconsistent phasing") + 
...
save_plot("view_examine_error/plots/introduced_error.pdf", 
          plot_grid(p1),
          base_width=3.5, base_height=2.15, base_asp=NULL)
